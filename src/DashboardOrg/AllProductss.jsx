...
const AllProductss = () => {
    ...
    const loadProducts = async () => {
        try {
            const res = await axios.get("https://server-gopts-bzds.vercel.app/admin-products/all", {
    ...
    const deleteProduct = async (id) => {
        ...
        try {
            await axios.delete(`https://server-gopts-bzds.vercel.app/admin-products/${id}`, {
                headers: { "x-email": user.email },
            });
    ...
    const toggleShowOnHome = async (id, value) => {
        try {
            await axios.patch(
                `https://server-gopts-bzds.vercel.app/admin-products/home/${id}`,
                { showOnHome: value },
                { headers: { "x-email": user.email } }
            );
            setProducts(
    ...
    const toggleHide = async (id, value) => {
        try {
            await axios.patch(
                `https://server-gopts-bzds.vercel.app/admin-products/hide/${id}`,
                { isHidden: value },
                { headers: { "x-email": user.email } }
            );
            setProducts(
